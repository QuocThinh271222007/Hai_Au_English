<?php
$pageTitle = 'Đăng nhập - Hải Âu English';
$currentPage = 'login';
...
<body class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
    <div class="min-h-screen flex justify-center px-4">
        <div class="max-w-md w-full">
            ...
            <!-- OAuth Error Message -->
            <div id="oauth-error-message" class="hidden mb-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg">
                <span id="oauth-error-text"></span>
            </div>

            <!-- OAuth Success Message -->
            <div id="oauth-success-message" class="hidden mb-4 p-4 bg-green-100 border border-green-400 text-green-700 rounded-lg">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8">
                ...
                <p class="mt-6 text-center text-sm text-gray-600">
                    Chưa có tài khoản? 
                    <a href="<?php echo $paths['signup']; ?>" class="text-blue-600 hover:text-blue-700 font-medium">
                        Đăng ký ngay
                    </a>
                </p>
            </div>
    ...
    <script>
        ...
        function loginWithGoogle() {
            if (!oauthConfig || !oauthConfig.google || !oauthConfig.google.enabled) {
                popup('Google OAuth chưa được cấu hình', 'error');
                return;
            }
        ...
        function loginWithFacebook() {
            if (!oauthConfig || !oauthConfig.facebook || !oauthConfig.facebook.enabled) {
                popup('Facebook OAuth chưa được cấu hình', 'error');
                return;
            }
        ...
        function handleOAuthResponse() {
            ...
            if (error) {
                const errorMessages = {
                    'oauth_cancelled': 'Đăng nhập bị hủy',
                    'invalid_state': 'Phiên đăng nhập không hợp lệ. Vui lòng thử lại.',
                    'oauth_failed': 'Đăng nhập thất bại. Vui lòng thử lại.'
                };
                
                const errorText = errorMessages[error] || 'Có lỗi xảy ra';
                document.getElementById('oauth-error-text').textContent = errorText;
                document.getElementById('oauth-error-message').classList.remove('hidden');
            ...
            if (oauth === 'success') {
                document.getElementById('oauth-success-message').classList.remove('hidden');
                
                // Redirect to profile after short delay
                setTimeout(() => {
                    window.location.href = '<?php echo $paths['profile']; ?>';
                }, 1500);
            }
        }
